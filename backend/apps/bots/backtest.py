...
def run(
    *,
    ...
    assumptions = Assumptions(
        ...
        fee_bps=decimal_setting("BACKTEST_FEE_BPS"),
    ...
    runtime = Runtime(
        result.program, symbol=symbol.upper(), inputs=inputs or {}, limits=limits()
    )
    engine = _Engine(
        runtime=runtime,
        assumptions=assumptions,
...
class _Engine:
    ...
    def _execute(self, pending, bar: Bar) -> None:
        ...
        if price <= ZERO or notional <= ZERO:
            return
        qty = notional / price
        fee = notional * self.a.fee_bps / BPS
        self.equity -= fee
    ...
    def _close(self, bar: Bar, price: Decimal, reason: str) -> None:
        ...
        exit_fee = price * position.qty * self.a.fee_bps / BPS
    ...
    def _slipped(self, price: Decimal, *, closing: bool, side: Side | None = None) -> Decimal:
        ...
        drift = price * self.a.slippage_bps / BPS
        if closing:

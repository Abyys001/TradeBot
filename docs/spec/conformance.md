...
| SL/TP are **mandatory** on every entry and every amend under the `protected` exit policy — the default, and every row written before Q37 — and are sent to the exchange | `order_views._percent` (400 without either), `executor._require_protection` (no account is touched), then real trigger prices per account: on the entry order where the venue accepts them (`native_sltp_on_entry`), placed and read back immediately after the fill where it does not (`executor._protect`) | `test_an_order_without_protection_is_refused`, `test_an_amend_without_protection_is_refused`, `test_an_intent_without_protection_never_reaches_an_exchange`, `test_paper_read_back_confirms_sl_and_tp_on_entry`, `test_the_default_is_the_rule_as_it_was`, `test_the_protected_policy_still_refuses_before_any_account_is_touched`, `test_the_api_still_refuses_a_bare_order_by_default` | ✅ |
| Under `strategy_managed` (Q37) the percentages are optional and the **strategy's exit signal closes the position at any PnL** — no threshold is consulted anywhere on that path | `apps/trading/protection.py` is the one door every caller comes through; `translate.plan` turns `desired_side=None` into a `CLOSE` against what the exchange holds; `apps/signals/` parses `BUY`/`SELL`/`EXIT_BUY`/`EXIT_SELL` into the same `StrategyIntent` a Pine bar produces | `test_an_exit_closes_with_no_percentages_configured_anywhere`, `test_an_exit_closes_a_losing_position_just_the_same`, `test_a_strategy_managed_entry_routes_with_no_protection_at_all`, `test_an_exit_closes_the_position_with_no_percentages_anywhere` | ✅ |
| A `strategy_managed` position's optional **safety net** rests at the venue only where no real stop does, and never past liquidation | `protection.Protection.resting_sl`, `protection.net_reachable` (basis-aware), refused at the form *and* at the intent | `test_the_safety_net_rests_only_where_no_real_stop_does`, `test_a_real_stop_wins_over_the_net_so_only_one_ever_rests`, `test_a_net_past_liquidation_is_named_rather_than_left_to_fail_per_leg`, `test_a_net_past_liquidation_is_a_400_naming_the_field` | ✅ |
| An external signal source reaches the exchange through **the order path that already exists**, never a second one | `signals.service.apply` builds a `StrategyIntent` and calls `translate.plan` / `RiskGate` / `translate.dispatch` / `route_*`; a payload naming `qty`, `leverage`, `price` or an account is refused by name | `test_a_buy_opens_a_position_through_the_ordinary_order_path`, `test_a_payload_may_not_set_what_belongs_to_the_platform`, `test_a_reversal_arrives_as_one_signal_and_leaves_as_two_sequenced_actions` | ✅ |
| The inbound signal endpoint carries, inside each request, every control the panel's session normally provides | `signals.auth` (HMAC-SHA256 over the raw body, signed timestamp, replay window, per-source IP allowlist), `UNIQUE (source, signal_id)` for duplicates, one 404 for unknown *and* disabled | `test_a_wrong_signature_is_refused_and_recorded`, `test_a_stale_delivery_is_refused_even_with_a_valid_signature`, `test_the_same_signal_id_twice_is_handled_once`, `test_a_disabled_source_answers_exactly_like_an_unknown_one` | ✅ |
| The §7 halt stops a webhook bot too, which has no evaluation task to cancel | `supervisor.stop_all` reads `Bot.state` rather than the task map (as `stop_all_sync` already did) | `test_the_halt_stops_a_webhook_bot_like_any_other`, `test_stop_all_is_callable_from_the_event_loop_too` | ✅ |
| SL/TP editable — (1) order entry | `Ticket.vue`, as a percentage or as an absolute price | ✅ |
| SL/TP editable — (2) from the chart | `useChartAdapter` draggable lines; chart pan frozen during a drag, 14px grab band (28px touch) | ✅ |
| SL/TP editable — (3) position row under the chart | `components/terminal/PositionBar.vue`, percentage or price | ✅ |
| …all three agree | All three write `stores/order.ts`; one write path, one fan-out | ✅ |
| …and all three **amend** mid-trade, not just the chart | `composables/useSltpAmend.ts` — one push path, one in-flight fan-out, last edit wins. The ticket used to accept a new level while a position was live and send it nowhere | ✅ |
| A live position is amendable from any tab | `stores/positions.ts` adopts the polled trade into `trading.tradeId` and `order.hydratedTradeId`, so a trade opened in another browser (or restored by `possync`) can be moved rather than being drawn and frozen | ✅ |
| An amend that missed one exchange is named | `market_views._sltp_stale` compares each leg's *resting* trigger against the trade's percentages under the same anchor/basis the amend uses; `PositionsTable.vue` badges it, `PositionBar.vue` offers the re-send | `test_positions_endpoint_names_the_leg_an_amend_did_not_reach`, `test_a_leg_with_no_protection_read_back_is_not_called_stale` | ✅ |
...
| Mid-trade change propagates ≤ 4s (Q19) | `fan_out(timeout=FANOUT_TIMEOUT_SECONDS)`, `route_amend` | `test_the_fanout_duration_is_recorded_for_audit` | ⚠️ **amended**: the 1s cap became `FANOUT_TIMEOUT_SECONDS` (default 10.0, via 4.0/3.0/5.0) after VPS round trips blew it on healthy orders. See `docs/decisions.md` Q19 |
...
| **Q24** the v1 subset only (user types, methods and enums included — see `docs/decisions.md` Q24 amendment); everything else rejected **by name, line and column**; nothing silently ignored | `apps/pine/subset.py:REJECTIONS`, `apps/pine/validate.py`, `apps/pine/objects.py` | `test_every_rejected_fixture_names_its_construct` and `test_every_rejection_has_a_fixture`, over 44 fixtures | ✅ |
...
2. **Q29's exports are not committed** — every `ta.*` golden value comes from
   TradingView (decided), and none has been exported yet. The

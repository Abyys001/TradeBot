...
<template>
  <div class="space-y-3 border-t border-border pt-3">
    <h4 class="text-xs font-medium text-fg-muted">{{ t('optimizer.title') }}</h4>
    <div class="flex gap-1">
      <button
        ...
        :class="tab === tname ? 'bg-accent/15 text-accent' : 'bg-surface-raised text-fg-muted'"
        @click="tab = tname"
      >
    ...
    <div v-if="tab !== 'montecarlo'" class="grid grid-cols-3 gap-1">
      <input v-model="coin" class="rounded border border-border bg-surface-muted px-2 py-1 text-[10px]" />
      <input v-model="interval" class="rounded border border-border bg-surface-muted px-2 py-1 text-[10px]" />
      <select v-model="network" class="rounded border border-border bg-surface-muted px-2 py-1 text-[10px]">
    ...
    <textarea
      ...
      rows="2"
      class="w-full rounded border border-border bg-surface-muted px-2 py-1 text-[10px] font-mono"
      :placeholder="t('optimizer.paramGrid')"
    />
    <div v-if="tab === 'montecarlo'" class="space-y-1">
      <select
        v-model="selectedBacktestId"
        class="w-full rounded border border-border bg-surface-muted px-2 py-1 text-[10px]"
      >
    ...
    <button
      type="button"
      class="w-full rounded bg-accent hover:bg-accent text-xs py-1.5 disabled:opacity-50"
    ...
    <p v-if="optimizer.error" class="text-[10px] text-negative">{{ optimizer.error }}</p>
    <GridResultsTable v-if="tab === 'grid' && optimizer.gridResults.length" :results="optimizer.gridResults" />
    <div v-if="tab === 'walkforward' && optimizer.walkForwardWindows.length" class="scrollbar-styled scrollbar-thin text-[10px] text-fg-muted space-y-1 max-h-32 overflow-y-auto">

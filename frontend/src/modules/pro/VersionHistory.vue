...
<template>
  <div class="border-t border-border pt-3 space-y-2">
    <div class="flex items-center justify-between">
      <h4 class="text-xs font-medium text-fg-muted">{{ t('versions.title') }}</h4>
      <button type="button" class="text-[10px] text-accent hover:underline" @click="snapshot">
        {{ t('versions.snapshot') }}
      </button>
    </div>
    <div v-if="!pro.versions.length" class="text-[10px] text-fg-muted">{{ t('versions.empty') }}</div>
    <ul v-else class="scrollbar-styled scrollbar-thin space-y-1 max-h-24 overflow-y-auto">
      <li
        v-for="v in pro.versions"
        :key="v.id"
        class="flex items-center justify-between text-[10px] text-fg-muted"
      >
        <span>v{{ v.version }} · {{ new Date(v.created_at).toLocaleDateString() }}</span>
        <button type="button" class="text-accent hover:underline" @click="restore(v.version)">
          {{ t('versions.restore') }}
        </button>
      </li>
    </ul>

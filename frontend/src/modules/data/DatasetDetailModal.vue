...
<template>
  ...
  >
    <div class="scrollbar-styled scrollbar-thin w-full max-w-lg rounded-xl border border-border bg-surface shadow-xl max-h-[80vh] overflow-y-auto">
      <div class="flex items-center justify-between border-b border-border px-4 py-3">
        <h3 class="text-sm font-medium text-fg">
          {{ dataset.coin }} / {{ dataset.interval || dataset.kind }}
        </h3>
        <button type="button" class="text-fg-muted hover:text-fg" @click="emit('close')">×</button>
      </div>
      <div class="p-4 space-y-4 text-sm">
        <div v-if="loading" class="text-fg-muted">{{ t('data.loading') }}</div>
        <template v-else>
          <div v-if="coverage" class="space-y-1">
            <div class="text-xs text-fg-muted">{{ t('data.detailCoverage') }}</div>
            <div class="text-fg font-mono text-xs">{{ JSON.stringify(coverage, null, 2) }}</div>
          </div>
          <div v-if="quality" class="space-y-1">
            <div class="text-xs text-fg-muted">{{ t('data.detailQuality') }}</div>
            <div class="text-fg font-mono text-xs">{{ JSON.stringify(quality, null, 2) }}</div>
          </div>
          <div v-if="gaps.length" class="space-y-1">
            <div class="text-xs text-fg-muted">{{ t('data.detailGaps', { count: gaps.length }) }}</div>
            <ul class="scrollbar-styled scrollbar-thin text-xs text-warning space-y-1 max-h-32 overflow-y-auto">
          ...
          <div v-if="funding.length" class="space-y-1">
            <div class="text-xs text-fg-muted">{{ t('data.detailFunding', { count: funding.length }) }}</div>
            <div class="scrollbar-styled scrollbar-thin text-xs text-fg-muted max-h-40 overflow-y-auto font-mono">
              <div v-for="(f, i) in funding.slice(-10)" :key="i">
                {{ new Date(f.time * 1000).toISOString().slice(0, 16) }}: {{ f.funding_rate.toExponential(3) }}
              </div>
            </div>
          </div>
          <div v-if="!gaps.length && !quality && !coverage && !funding.length" class="text-fg-muted text-xs">
            {{ t('data.detailEmpty') }}
          </div>
        </template>
      </div>

...
function pairBadgeClass(status: string) {
  if (status === 'done') return 'bg-emerald-900/50 text-emerald-300 border border-emerald-900/40'
  if (status === 'partial') return 'bg-amber-900/50 text-amber-300 border border-amber-900/40'
  if (status === 'empty') return 'bg-zinc-800 text-zinc-400 border border-zinc-700'
  if (status === 'skipped') return 'bg-zinc-800 text-zinc-500 border border-zinc-700'
  if (status === 'failed') return 'bg-red-900/50 text-red-300 border border-red-900/40'
  if (status === 'queued') return 'bg-zinc-800/60 text-zinc-500 border border-zinc-700/50'
  if (status === 'running') return 'bg-amber-900/50 text-amber-300 border border-amber-700/50 animate-pulse'
  return 'bg-amber-900/50 text-amber-300 border border-amber-700/50 animate-pulse'
}
...
function statusCfg(job: HistoryDownload) {
  ...
  if (isStale) return { label: 'Stale', cls: 'text-amber-400', bar: 'bg-amber-500', ping: false }
  if (job.status === 'done') return { label: 'Complete', cls: 'text-emerald-400', bar: 'bg-emerald-500', ping: false }
  if (job.status === 'partial') return { label: 'Partial', cls: 'text-amber-400', bar: 'bg-amber-500', ping: false }
  if (job.status === 'failed') return { label: 'Failed', cls: 'text-red-400', bar: 'bg-red-500', ping: false }
  if (job.status === 'running') return { label: 'Downloading…', cls: 'text-amber-300', bar: 'bg-amber-400', ping: true }
  return { label: 'Pending', cls: 'text-zinc-400', bar: 'bg-zinc-600', ping: false }
}
...
<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      ...
      <span v-if="sorted.length" class="text-xs text-zinc-500">{{ sorted.length }} jobs</span>
    </div>

    <div
      v-if="!sorted.length"
      class="rounded-xl border border-dashed border-zinc-800 py-10 text-center text-sm text-zinc-500"
    >
      {{ t('data.noJobs') }}
    </div>
    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        <button
          type="button"
          class="w-full text-start px-4 py-3 hover:bg-zinc-900/40 transition-colors"
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="text-xs text-zinc-400 truncate">
                  ...
                  <span class="text-zinc-600"> · {{ job.network }}</span>
                </span>
                <span class="ml-auto text-[11px] text-zinc-600 shrink-0">
              ...
              <div class="mt-2">
                <div class="flex items-center justify-between text-[11px] mb-1">
                  <span class="text-zinc-400">{{ progressText(job) }}</span>
                  <span v-if="job.status !== 'done' && progressPct(job) > 0" class="text-zinc-500">
                    {{ progressPct(job) }}%
                  </span>
                </div>
            ...
            <button
              v-if="canRetry(job)"
              type="button"
              class="shrink-0 rounded px-2.5 py-1 text-xs bg-zinc-800 text-zinc-300 hover:bg-zinc-700 disabled:opacity-50"
            ...
            <!-- chevron -->
            <svg
              class="shrink-0 h-4 w-4 text-zinc-600 transition-transform duration-200"
        ...
        <!-- expanded pair grid -->
        <div v-show="isExpanded(job)" class="border-t border-zinc-800/50 bg-zinc-900/20 px-4 py-3 space-y-3">
          <p v-if="job.error" class="text-xs text-red-300 font-mono bg-red-950/30 rounded px-2 py-1.5">
            {{ job.error }}
          </p>
          ...
          <div v-if="Object.keys(job.progress || {}).length" class="flex flex-wrap gap-2">
            ...
            >
              ...
              <span v-if="entry.error" class="text-[10px] text-red-300 max-w-[120px] truncate">{{ entry.error }}</span>
            </div>
          </div>

          <p v-else-if="job.status === 'running'" class="text-xs text-amber-400 animate-pulse">
            Initializing pairs…
          </p>

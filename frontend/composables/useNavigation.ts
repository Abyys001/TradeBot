...
export function useNavigation() {
  ...
  const items = computed<NavItem[]>(() => [
    ...
    // /strategies is deliberately **not** in the sidebar. The editor is reached
    // from the bots list and the backtest form, which is where a script is
    // actually wanted; a top-level entry made it look like a second runtime.
    {
      name: 'risk',

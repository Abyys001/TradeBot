...
profit split), **bot mode** — a Pine Script **v5/v6** engine (the v1 subset
**plus user-defined types, methods and enums**, and plus what a *published*
...
backtest, supervisor and panel (`docs/bots.md`) — **with one warm-up shared by
the live loop and the backtest** (`feed.strategy_warmup`; when they differed, a
bot traded a strategy its own report had never described, Q38) — gated
...
`apps/signals/`, joined to the *same* order path a Pine bar takes), and **a
manual control tab for when TradingView and the bot disagree** (Q41: open or
close this bot's position by hand, on that same order path, with the bot still
running the strategy and its own next exit signal closing what was opened by
hand — and a hand-*closed* side not re-entered until the script stops asking
and asks again) — and an
...
| `apps/bots/chart.py` | **The chart tab, and why it is not a log.** **One `backtest.run` per bot, and a page is a slice of it** (Q40): the arrows are where this strategy would have entered and exited, at the fill model's own price, whether or not a bot was running then. A page-sized replay of its own started *flat* on the page's left edge — mid-campaign that opens a position TradingView never opened and sizes every one after it off the wrong equity, and two pages then disagreed about the same bar. `BotAction` rows are drawn beside the replay's marks and `summary.unrouted` counts the entries nothing was sent for — per **campaign**, since a long that scaled out three times is one decision. Marks are per campaign too, labelled with the script's own `comment=` (TradingView's Signal column), and a `plotshape` is a **mark, not a line**: its series is a condition, and `float(False)` drawn on the price scale is a flat run at zero that leaves the candles a sliver. `before=` pages backwards through the cached replay, and its oldest bar is `no_history` rather than an error. |
...
| `apps/bots/intervene.py`, `desk.py` | **The operator's own hand, and the evidence for using it (Q41).** TradingView and the bot come apart — the bot was stopped when the signal fired, the feed was repairing a gap, a bar arrived late — and waiting for the *next* signal means sitting out the trade the strategy is already in. `intervene.act` is the door for "do it anyway", and **not a second order path**: a press becomes the same `StrategyIntent` a Pine bar produces and goes through the same `translate.plan` → `RiskGate` → `route_*`, so the halt still refuses an entry and sizing is still §5. A hand-opened trade is stamped with the run, so `sync_position` tells the runtime about it and the script's own exit closes it. A hand-*closed* one is guarded: the entry the operator overrode is the one **still being asked for**, so `decide` waits for the script to stop asking and ask again — HOLDING → ARMED → released — which is what "the next valid signal" means when the signal is a condition rather than an event. Paper is a property of the **position**, not of `dry_run`: `supervisor.stop` sets that flag on every bot it stops, so a bot that stopped itself while holding would otherwise have its close recorded as a would-have-been and answered `ok`. `desk.py` is the tab's single read — what the script last said, what is held marked to market, the money over the accounts this bot can actually reach, and the divergence between the first two as a code. Derives, never decides. |
| `apps/bots/exitguard.py` | **The exit the platform takes itself (Q38).** A stop or target sent to a venue and no longer there looks exactly like one quietly waiting — there is no callback — so the level is watched from this side too, off the `Trade` row's own recorded percentages, and a bar that reaches it becomes an ordinary `CLOSE` on the one order path. It is the backtest's rule (`_check_exits`), not a second one, so switching it on moves live *towards* the report. It can invent nothing: a trade with no level recorded gets none enforced. One verb, one direction: out. |
...
| `pages/bots/index.vue`, `pages/bots/[id].vue` | The bots, stopped ones first: Q25's premise is that nobody is watching at 03:00, so a bot that stopped itself does not sort under the running ones where it reads as idle. The detail page's spine is the promotion gate — measurements rather than a confirmation dialog, each row waivable and the whole gate switchable off, because who may promote an untested bot is the admin's call and not this platform's. The header line (`BTCUSDT 1h · 1×`) is the edit button: everything a bot *is* except the version it pins, which is its identity. |
| `components/bots/InputsForm.vue`, `StrategyInputs.vue`, `InputsDialog.vue` | **The other half of that dialog — the script's own settings, and the only editable tab whose values reach live untouched.** Every row is drawn from the version's stored schema: the groups the author wrote, in the order they wrote them, `inline=` runs on one line, the control the type asks for, and a note saying which toggle turns the row off. It recomputes nothing — the effective value, the widget, the category and the gate all arrive resolved, because a browser holding a second opinion about which of script-or-panel won a field is how the report and the form that produced it start disagreeing. Saving is a diff: "reset" deletes the key rather than writing the default back, so a later version moving that default still reaches the bot. |
...
| `utils/clock.ts` | **One clock: `Europe/London`.** Both charts' axes and crosshairs, the trade log, the journal and the activity list, all on the zone the admin's TradingView is set to — never the browser's. Nothing is *shifted*: Lightweight Charts has no timezone and the usual workaround moves every value by the offset, leaving the chart holding numbers that are not the instants they claim to be. The formatters are replaced instead. BST comes from `Intl` with the IANA zone, not a `+1` constant. |
| `components/bots/BotDesk.vue` | **Manual control (Q41), where the decision and the buttons are one screen.** The verdict comes first — aligned, or the exact way the chart and the platform differ — and everything under it is the evidence for that line: what the strategy last said and whether that bar was an entry *signal* or a position persisting, the position per leg, total capital and free balance over the **eligible** accounts, PnL, the mark, and the tail of what was actually sent. It recomputes nothing; `apps/bots/desk.py` prices all of it. Two clicks to send, because a market order across every partner account is not an undo, and a dead button says *why* rather than leaving it to be discovered by pressing it. A refusal is an answer: the code comes back translated, and the server's own sentence is shown when there is no key for it. |
| `components/bots/BotJournal.vue`, `BotChart.vue`, `BotLogic.vue`, `BotActivity.vue` | **What the bot is doing, in four tabs.** The journal renders `narrate.py`'s codes through i18n and folds the quiet bars behind one toggle — every evaluated bar gets a line, because "no bars recorded yet" on a running bot reads as a bot that is not working. The chart is a **visual backtest** (Q39, Q40): `apps/bots/chart.py` replays the venue's own bars **once**, continuously, and every page is a slice of that one run — so the position, the size and the running total on a bar are the Strategy Tester's and not a fresh start at the left edge. The arrows are where the strategy *would* have traded whether or not the bot was running then, carrying the script's own labels; dragging left fetches the page before, and the last bar ticks off the socket. Its timeframe selector **re-replays for display only**: picking an interval the bot does not run on changes nothing about the running bot and says so on the badge. Its canvas host is mounted unconditionally with the loading and empty states drawn *over* it — Lightweight Charts measures the element it is handed, so a container behind a `v-if` on `loading` is a chart that is never created. The logic tab is `explain.py`. `BotActivity.vue` is the routed orders and the accounts each reached, shown both in its own tab and beside the chart — across every run, because a restart is not the start of the bot's history. There is no bars tab: "no actions" and "no bars either" are one question, so the activity tab carries the evaluated-bar count in its empty state. |
| `components/ui/Progress.vue`, `SymbolPicker.vue` | The two pieces the backtest form needed: a bar that reports the phase it is in (a first run is mostly downloading, and a spinner there is indistinguishable from a hang), and a searchable pair picker over the downloaded catalogue that still accepts free text — the archive is not the list of pairs that exist. |
...
| `components/telegram/TelegramCard.vue`, `stores/telegram.ts` | The Telegram notifier on `/settings` (Q36): write-only token, recipient, deep-link linking, language, one checkbox per event group, a test message, and the service's heartbeat — shown because a notifier that died is indistinguishable from a quiet book. Step-up replays like the security card. |

...
| `components/bots/BotJournal.vue`, `BotChart.vue`, `BotLogic.vue`, `DrillPanel.vue` | **What the bot is doing, in four tabs.** The journal renders `narrate.py`'s codes through i18n and folds the quiet bars behind one toggle — every evaluated bar gets a line, because "no bars recorded yet" on a running bot reads as a bot that is not working. The chart draws the script's own plotted series and its signals as markers, and its timeframe selector **re-replays for display only**: picking an interval the bot does not run on changes nothing about the running bot and says so on the badge. The logic tab is `explain.py`. The drill panel fires the gate's two exercises and warns first, because both send real orders. |

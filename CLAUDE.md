...
| `apps/bots/chart.py` | **The chart tab, and why it is not a log.** One page = one `backtest.run` over the venue's own bars for that window: the arrows are where this strategy would have entered and exited, at the fill model's own price, whether or not a bot was running then. `BotAction` rows are drawn beside them as their own marks and `summary.unrouted` counts the entries nothing was sent for — the execution discrepancy, as a number rather than something to spot by eye. `before=` pages backwards, each page warmed by its own lead-in bars, and the end of the venue's history is `no_history` rather than an error. |

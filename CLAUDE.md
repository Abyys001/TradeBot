...
**~2330 backend tests pass** (the DB-backed ones need Postgres; `./run.sh
...
| `components/bots/BotJournal.vue`, `BotChart.vue`, `BotLogic.vue` | **What the bot is doing, in three tabs.** The journal renders `narrate.py`'s codes through i18n and folds the quiet bars behind one toggle — every evaluated bar gets a line, because "no bars recorded yet" on a running bot reads as a bot that is not working. The chart draws the script's own plotted series and its signals as markers, and its timeframe selector **re-replays for display only**: picking an interval the bot does not run on changes nothing about the running bot and says so on the badge. Its canvas host is mounted unconditionally with the loading and empty states drawn *over* it — Lightweight Charts measures the element it is handed, so a container behind a `v-if` on `loading` is a chart that is never created. The logic tab is `explain.py`. There is no bars tab: "no actions" and "no bars either" are one question, so the activity tab carries the evaluated-bar count in its empty state. |
